...
#Response=np.ceil(Response[0:ct]/30)
#Response=np.round(Response/30)
...
if(XX==1):
    ...
    #    plt.plot(X_test, y_dt, c="g", label="n_estimators=1", linewidth=2)
    #    plt.plot(X_test, y_adadt, c="r", label="n_estimators=300", linewidth=2)
    #    plt.xlabel("data")
    #    plt.ylabel("target")
    ...
    #    mdl_br = linear_model.BayesianRidge()
    ...
    # R2 calculation in Python is not correct, I recalculated in excel
    r2_score_svr=0.7176
    r2_score_en=0.7144
    r2_score_regrf=0.79
    r2_score_plsr=0.6954
    ...
    #plt.plot(Pred_test[0,:],Pred_test[8,:],'.',ms=8,c='r',label='PLSR: $R^2$ = %.2f' %r2_score_plsr)

    #
    #plt.plot(Pred_test[0,:],Pred_test[5,:],'.',ms=10,c='c',label='ADADT: $R^2$ = %.2f' %r2_score_adadt)
    ...
    #plt.legend()
    plt.show()
    fig1.savefig('tmix_ML_24jan19.tif',facecolor='white', edgecolor='white')
...
#Response=stats.zscore(Response,axis=0,ddof=1)
Response=(Response-mean_resp1)/std_resp1

#svr_lin = SVR(kernel='linear',C=1)
#mdl_svr = svr_lin.fit(Data,Response)
#Coeff_SVR=mdl_svr.coef_

#t_data=np.nan_to_num(t_data)
#p_test1=mdl_svr.predict(t_data)*std_resp + mean_resp
#print(p_test1)
...
t_resp=np.zeros((16,1))

svr_lin = SVR(kernel='linear',C=1)
svr_lin.fit(Data,Response)

for i in range(0,10):
    t_data=[6000,60000,0.7,0.6,5e+07,1800,56938,5,10.17]
    #t_data[8]=(i+10)*2
    #t_data[6]=t_data[6]*10.17/t_data[8]      
    t_data[7]=2*(i+1)
    t_data= (t_data-mean_data1)/std_data1
    t_resp[i,0]=svr_lin.predict(t_data)*std_resp1+mean_resp1
